...
- **오늘 접속 계정**: 관리자 상단 `오늘 N` 클릭 → ID(이메일)·소속·이름·최종 접속 시간 팝업. 한국 시간 기준 오늘만, 계정당 한 건. 기존 방문 통계는 게스트 포함이며 팝업은 로그인 계정만 표시합니다.
- **오락실**: 관리자·학생 공용 게임 목록. 관리자가 HTTPS URL·게임이름·게임설명으로 추가하면 아래로 누적되며, 썸네일/이름 클릭 시 LMS 내부 프레임에서 실행합니다. 예제 `https://jellyrungo.vercel.app/` 포함. 외부 게임은 프레임 실행을 허용해야 하며, 새 게임 썸네일은 기존 WordPress mshots 서비스를 사용합니다.
- 오락실·오늘 접속 권한 회귀 검사: `supabase db query --linked --file supabase/tests/arcade_today_smoke_test.sql` (테스트 데이터는 트랜잭션 롤백).

- **도움 요청 대기열**: 학생 상단 🙋 버튼(문제 유형·설명·스크린샷) → 관리자 상단 `도움 요청 N명` + 대기열 화면(접수 순서·대기시간, 처리 시작→해결, Realtime). 학생은 내 대기 순번/처리 상태를 실시간 확인·취소
- **학생 쪽지 삭제**: 쪽지함에서 개별/전체 삭제 (사용자별 숨김, `push_hidden`)
